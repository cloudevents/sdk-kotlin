 ...
 * `datacontenttype` of `application/cloudevents+protobuf` (parsed as `Any`) routes to `proto_data`;
...
public object ProtobufEventFormat {
     ...
     *   declared `application/cloudevents+protobuf` that is not a valid `Any`, or a text-declared
     ...
     *   (unparseable bytes, unknown `spec_version`, a contradictory `oneof`, or a semantic violation
     *   of the event's own version).
     */
